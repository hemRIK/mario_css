...
const velocidade = (contarcinco) =>
{
   if(contarcinco ==5) // numero de pulos = 5
   {
    contarcinco= 0;  // zerar contador 
    incremendoDeVelocidade  = incremendoDeVelocidade - 10;   // variavel que determinar a velocidade do jogo 
    cano.style.animation = `movercano ${incremendoDeVelocidade }ms infinite linear `  // muda a velocidade da animação do css 
    ...
    if( canoPosicao <= 120 && canoPosicao>0 && marioPosicao<80)   // se posição do cano for = 120 e o mario nao estiver no ar (altura do pulo = 80)
    {
       ...
       clearInterval(loop);
        ...
        { 
         ...
         velocidade(contarcinco)   //função contar 5 canos pulados e incremenda velocidade 
